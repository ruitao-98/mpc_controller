...
fig, ax = plt.subplots()
plt.grid(ls='--')
x = np.linspace(0,xx.shape[0])
trajectory = plt.plot(xx[:,0,0],xx[:,1,0],'red',alpha=0.5)
final_position = patches.Rectangle(((xx[-1,0,0] - 0.1/2), (xx[-1,1,0]- 0.1/2)), 0.1, 0.1, fc='b')
realtime_position = patches.Rectangle(((xx[0,0,0] - 0.1/2), (xx[0,1,0]- 0.1/2)), 0.1, 0.1, fc='r')
ax.add_patch(final_position)
ax.add_patch(realtime_position)
def animate(nums):
    #车辆平移
    realtime_position.set_xy((xx[nums,0,0] - 0.1/2, xx[nums,1,0] - 0.1/2))
    # 创建以方块中心为原点的旋转矩阵
    t = transforms.Affine2D().rotate_deg_around(xx[nums,0,0], xx[nums,1,0], (xx[nums,2,0] * 180 / 3.1415926))
    # 更新旋转矩阵
    realtime_position.set_transform(t + ax.transData)
    return [realtime_position]

# 创建动画对象，frames是帧数，interval是每帧之间的延迟(以毫秒为单位)，blit是一种优化选项，可以提高动画的渲染速度
ani1 = FuncAnimation(fig, animate, frames=xx.shape[0], interval=100, blit=True)
ani1.save('vehicles.gif')
