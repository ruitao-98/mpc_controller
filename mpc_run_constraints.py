...
import MPC_controller_Constraints as Cons
...
#定义系统约束上限
x_high = np.array([[5], [5], [5]])
#定义输入下限
u_low = np.array([[0], [0], [0]])
#定义输入上限
u_high = np.array([[5], [5], [5]])
...
#二次规划需用到的矩阵
M, Beta_bar, b = Cons.M_F_generator(x_low,x_high,u_low,u_high,N_P,Phi,Gamma)

for k in range(k_steps-1):
    u = contro.MPC_Controller_Constraints(x,F,H,M,Beta_bar,b,p)
